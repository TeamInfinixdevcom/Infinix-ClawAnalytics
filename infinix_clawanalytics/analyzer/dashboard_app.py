...
def run_dashboard(scored_path: str | Path = DEFAULT_SCORED_PATH) -> None:
    ...
    with st.sidebar:
        ...
        source_mode = st.selectbox("Modo", ["scored", "configured", "csv_wizard"], index=0)
        config_path = st.text_input("Config", value="config.yaml")
        source_override = st.selectbox("Tipo", ["(default)", "csv", "postgres", "mysql", "api"], index=0)
    ...
    if source_mode == "configured":
        ...
        st.subheader("Carga rapida de CSV")
        ...
        if st.button("Ejecutar pipeline con este CSV"):
            try:
    ...
    with st.sidebar:
        ...
        canal = st.multiselect("Canal", sorted(df["canal"].unique()))
        ejecutivo = st.multiselect("Ejecutivo", sorted(df["ejecutivo"].unique()))
